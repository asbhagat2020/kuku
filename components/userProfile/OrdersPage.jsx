...
const getDisplayStatus = (order) => {
  ...
  if (status === "Buyer Wanted Return Item") return "Return Request";
...
const OrdersPage = () => {
  ...
  // Check if order is within 24-hour return window → AB SAHI TIME SE!
  const isReturnable = (order) => {
    if (order.orderStatus !== "Delivered") return false;

    // Step 1: Pehle delivery ka exact timestamp nikaal lo history se
    let deliveryTime = null;

    // Option 1: orderStatusHistory se (recommended)
    const deliveredStatus = order.orderStatusHistory?.find(
      (h) => h.status === "Delivered"
    );
    if (deliveredStatus?.timestamp) {
      deliveryTime = new Date(deliveredStatus.timestamp);
    }

    // Option 2: Agar upar nahi mila to deliveryDetails se fallback
    if (!deliveryTime) {
      const deliveredDelivery =
        order.processingDetails?.deliveryDetails?.deliveryStatusHistory?.find(
          (h) => h.status === "Delivered"
        );
      if (deliveredDelivery?.timestamp) {
        deliveryTime = new Date(deliveredDelivery.timestamp);
      }
    }

    // Option 3: Agar dono fail → purana paidAt (safety)
    if (!deliveryTime) {
      deliveryTime = new Date(order.paidAt);
    }
  ...
  // Add this helper function upar, getDisplayStatus ke saath
  const getRelevantDate = (order, displayStatus) => {
    if (!displayStatus) return null;

    // 1. Delivered → Actual delivery time from history
    ...
    // 2. Cancelled → Jab cancel hua
    if (displayStatus === "Cancelled") {
      const cancelledEntry = order.orderStatusHistory?.find(
        (h) => h.status === "Cancelled"
      );
      if (cancelledEntry?.timestamp) {
        return new Date(cancelledEntry.timestamp);
      }
    }

    // 3. Returned / Rented Returned → Return delivered time
    if (displayStatus === "Returned" || displayStatus === "Rented Returned") {
      const returnedEntry = order.orderStatusHistory?.find(
        (h) =>
          h.status === "Return Delivered to Seller" ||
          h.status === "Rented Return Delivered"
      );
      if (returnedEntry?.timestamp) {
        return new Date(returnedEntry.timestamp);
      }
    }

    // 4. Confirmed → Payment time
    if (displayStatus === "Confirmed") {
      return order.paidAt ? new Date(order.paidAt) : null;
    }

    // 5. Pending → Order created time
