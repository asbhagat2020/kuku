...
// YE PROPS ADD KIYE HAIN — BAS YE HI CHANGE HAI (Baki sab 100% same)
export const SellingProducts = ({ 
  data, 
  activeFilter = "All",           // ← Ab ye DetailsSection se aayega
  setActiveFilter                 // ← Ye bhi parent se aayega (URL update ke liye)
}) => {
  const [isOfferPopupOpen, setIsOfferPopupOpen] = useState(false);
  const [offerSubmitted, setOfferSubmitted] = useState(false);
  const [selectedPrice, setSelectedPrice] = useState(null);
...
useEffect(() => {
  const hash = window.location.hash; // #product-68de8f63d28eacfe745c36db
  if (hash && hash.startsWith("#product-")) {
    setTimeout(() => {
      ...
      if (element) {
        ...
        // Highlight effect
        element.classList.add("ring-4", "ring-yellow-400", "ring-offset-4", "bg-yellow-50", "shadow-xl");

        // 4 second baad remove
        setTimeout(() => {
          element.classList.remove("ring-4", "ring-yellow-400", "ring-offset-4", "bg-yellow-50", "shadow-xl");
        }, 4000);
      }
    }, 800); // thoda delay taaki page load ho jaye
  }
}, [data]); // jab products load ho jaye tab check karega
        ...
        <div className="flex flex-wrap gap-3 p-4 bg-gray-50 rounded-lg">
          {filterOptions.map((option, index) => {
            ...
            return (
              <div key={option.value} className="flex items-center">
                <button
                  onClick={() => setActiveFilter(option.value)}  // ← Ye ab URL bhi update karega
                  className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 flex items-center gap-2 ${
                    isActive
                      ? "bg-custom-yellow text-black shadow-md"
                      : "bg-white text-gray-600 hover:bg-gray-100 border border-gray-200"
