...
export const DetailsSection = ({ data }) => {
  ...
  useEffect(() => {
    if (data?._id) {
      console.log("User ID for fetching products:", data._id);
      getProductsByUser();
    }
  }, [data]);
  ...
  // Update selected tab when tabs change (important for count updates)
  useEffect(() => {
    if (selectedTab.label === "Selling") {
      setSelectedTab(tabs[0]); // Update to get fresh count
    }
  }, [products]);
