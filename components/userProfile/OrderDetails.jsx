...
//   // Dynamic Progress Steps based on order status
...
//           order?.processingDetails?.deliveryDetails?.deliveryStatusHistory?.some(
//             (h) => h.status === "Out For Delivery"
//           ),
//       },
//       {
...
//       order?.orderStatus === "Out for reverse delivery"
//     ) {
//       steps.push({
//         title:
//           order?.orderStatus === "Returned"
//             ? "Returned"
//             : "Out for Reverse Delivery",
//         subtitle:
//           order?.processingDetails?.deliveryDetails?.deliveryStatusHistory?.find(
//             (h) =>
//               h.status === "RTO Delivered" ||
//               h.status === "Out For Reverse Delivery"
//           )?.timestamp
//             ? new Date(
//                 order.processingDetails.deliveryDetails.deliveryStatusHistory.find(
//                   (h) =>
...
//                   {order?.processingDetails?.deliveryDetails?.provider ||
//                     "Al ameen express"}
//                 </p>
//               </div>
//               <hr className="border-gray-200" />
...
//         <div className="flex flex-col md:flex-row gap-6">
//           <div
//             className={`bg-[${CONSTANTS.COLORS.ITEM_BG}] shadow-md border border-gray-200 p-6 rounded-lg flex-1`}
//           >
//             <div className="flex flex-col md:flex-row items-start gap-6 mb-6">
//               <img
//                 src={
...
//                         AED {order?.finalAmount?.toFixed(2) || "0.00"}
//                       </span>
//                       {order?.discount > 0 && (
//                         <span className="text-emerald-500 text-xs md:text-sm">
...
//                         MRP AED {order?.totalAmount?.toFixed(2) || "0.00"}
//                       </span>
//                     )}
//                   </div>
//                 </div>
//                 <p className="text-gray-500 text-xs md:text-sm mt-2">
//                   {order?.products?.[0]?.product?.description ||
//                     "No description"}
//                 </p>
//                 <p className="text-gray-500 text-xs md:text-sm mt-2">
//                   Brand:{" "}
//                   {order?.products?.[0]?.product?.brand?.brandName ||
//                     "Unknown Brand"}
//                 </p>
//                 <p className="text-gray-500 text-xs md:text-sm mt-2">
//                   Size: {order?.products?.[0]?.product?.size?.sizeName || "N/A"}
//                 </p>
//                 <p className="text-gray-500 text-xs md:text-sm mt-2">
//                   Condition:{" "}
//                   {order?.products?.[0]?.product?.condition?.conditionName ||
//                     "Unknown Condition"}
//                 </p>
//               </div>
//             </div>
...
//           <div className="mx-auto mb-4">
//             <div
//               className={`w-16 h-16 bg-[${CONSTANTS.COLORS.SUCCESS_BG}] rounded-full flex items-center justify-center mx-auto`}
//             >
//               <img
//                 src="/greencircle.png"
//                 alt="Green Circle"
  ...
  const getProgressSteps = () => {
    ...
    if (order?.products?.[0]?.isRental) {
      steps.push({
        ...
        subtitle: order?.processingDetails?.returnDetails?.returnStatusHistory?.find((h) => h.status === "Scheduled")?.timestamp
          ? new Date(order.processingDetails.returnDetails.returnStatusHistory.find((h) => h.status === "Scheduled").timestamp).toLocaleString()
          : "N/A",
        isCompleted: ["Scheduled", "Picked Up", "Delivered"].includes(order?.processingDetails?.returnDetails?.returnStatus),
      });
      steps.push({
        title: "Item Picked Up",
        subtitle: order?.processingDetails?.returnDetails?.returnStatusHistory?.find((h) => h.status === "Picked Up")?.timestamp
          ? new Date(order.processingDetails.returnDetails.returnStatusHistory.find((h) => h.status === "Picked Up").timestamp).toLocaleString()
          : "N/A",
        isCompleted: ["Picked Up", "Delivered"].includes(order?.processingDetails?.returnDetails?.returnStatus),
      });
