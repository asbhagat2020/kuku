...
export default function GoogleTranslate() {
  const [isOpen, setIsOpen] = useState(false);
  const [currentLang, setCurrentLang] = useState("EN");
  const [isTranslateAvailable, setIsTranslateAvailable] = useState(true);
  ...
  const handleLanguageChange = (langCode, googleCode) => {
    ...
    setTimeout(() => {
      if (googleCode === "en") {
        console.log("Resetting to English - Force reload with cleanup");
        window.location.hash = "";
        const domain = window.location.hostname;
        const cookieOptions = `expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/; domain=${domain}`;
        document.cookie = `googtrans=; ${cookieOptions}`;
        document.cookie = `googtrans=/auto/en; ${cookieOptions}`;
        document.cookie = `googtrans=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;`;
        document.cookie = `googtrans=/auto/en; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;`;
        ...
        try {
          if (typeof Storage !== "undefined") {
            ...
            lsKeys.forEach((key) => {
              if (key.includes("goog") || key.includes("translate")) {
  ...
  useEffect(() => {
    ...
    const addGoogleTranslateScript = () => {
      ...
      const loadScript = () => {
        console.log(`Loading Google Translate script (Attempt ${attempts + 1})`);
        const script = document.createElement("script");
        ...
        script.onerror = () => {
          ...
          if (attempts < maxAttempts) {
            ...
            setIsTranslateAvailable(false);
            fetch("https://translate.google.com")
              .then(() => alert("Translation service is currently unavailable. Please try again later."))
              .catch(() => alert("Translation service is blocked. Please disable your ad blocker and try again."));
          }
        };
        script.onload = () => {
      ...
      window.googleTranslateElementInit = () => {
        ...
        try {
          ...
          console.error("Error initializing Google Translate:", error);
          setIsTranslateAvailable(false);
          alert("Failed to initialize translation service.");
        }
      };
    };
    ...
    const checkForTranslateReady = () => {
      ...
      const interval = setInterval(() => {
        ...
        if (selectElement) {
          console.log("Google Translate is ready");
          clearInterval(interval);
          setTimeout(() => checkCurrentLanguage(), 1500);
          setTimeout(() => {
            const elementsToHide = [
              ".goog-te-banner-frame",
              ".skiptranslate",
  ...
  return (
    <div className="relative">
      <button
        onClick={() => isTranslateAvailable && !isLoading && setIsOpen(!isOpen)}
        className={`flex items-center gap-2 px-4 py-2 bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/20 rounded-lg text-white font-medium transition-all duration-200 hover:scale-105 active:scale-95 notranslate ${
          !isTranslateAvailable || isLoading ? "opacity-50 cursor-not-allowed" : ""
        }`}
        disabled={!isTranslateAvailable || isLoading}
      >
        <Globe className="w-4 h-4" />
        <span className="text-sm">{getCurrentLanguage()}</span>
        {isLoading ? (
            ...
            <div className="space-y-1 max-h-64 overflow-y-auto">
              {languages.map((lang) => (
                <button
                  key={lang.code}
                  className={`w-full flex items-center justify-between px-3 py-2.5 text-sm rounded-lg transition-all duration-150 notranslate ${
                    currentLang === lang.code ? "bg-blue-50 text-blue-700 border border-blue-200" : "text-gray-700 hover:bg-gray-50"
                  }`}
                  onClick={() => handleLanguageChange(lang.code, lang.googleCode)}
                >
                  <div className="flex items-center gap-3">
                    <span className="font-medium text-xs bg-gray-100 px-2 py-1 rounded notranslate">{lang.code}</span>
                    <span className="notranslate">{lang.name}</span>
                  </div>
                  {currentLang === lang.code && <div className="w-2 h-2 bg-blue-500 rounded-full"></div>}
                </button>
              ))}
            </div>
          </div>
