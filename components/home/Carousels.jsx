...
const Carousels = () => {
  ...
  const isOwnProduct = (item) => {
    return item?.seller?._id === userID;
  };
                    ...
                    {token ? (
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          router.replace(
                            `/login?redirect=/selling-page/${item._id}`
                          );
                  ...
                  <div className="absolute min-w-[204px] bottom-4 left-4 text-center z-10 bg-[#fde504] px-[50px] py-[20px] rounded-[20px]">
                    <button
                      onClick={(e) => {
                        ...
                        // If not logged in — SAME AS COMMENTED CODE
                        if (!token) {
                          router.replace(
                            `/login?redirect=/selling-page/${item._id}`
                          );
                        ...
                        // Logged in → go to product page
                        router.push(`/selling-page/${item._id}`);
                      }}
                      className="text-[#202020] text-base font-bold font-karla leading-tight"
                    >
                      Buy Now
