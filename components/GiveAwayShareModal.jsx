...
// const GiveAwayShareModal = ({ isOpen, onClose, giveawayDetails }) => {
//   const [copySuccess, setCopySuccess] = useState(false);
//   const [giveawayUrl, setGiveawayUrl] = useState("");

//   // Generate giveaway share URL when component mounts on client
//   useEffect(() => {
//     if (typeof window !== "undefined") {
//       const url = `${window.location.origin}/giveaway/${giveawayDetails?.id || "shared"}`;
//       setGiveawayUrl(url);
//     }
//   }, [giveawayDetails?.id]);

//   const shareText = `Check out this giveaway on Kuku! ${giveawayDetails?.items?.join(", ") || "Items"} available for pickup.`;
...
//     const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(shareText + " " + giveawayUrl)}`;
...
//     const twitterUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(shareText)}&url=${encodeURIComponent(giveawayUrl)}`;
//     window.open(twitterUrl, "_blank");
//   };

//   const handleFacebookShare = () => {
//     const facebookUrl = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(giveawayUrl)}`;
//     window.open(facebookUrl, "_blank");
//   };

//   const handleLinkedInShare = () => {
//     const linkedinUrl = `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(giveawayUrl)}`;
//     window.open(linkedinUrl, "_blank");
//   };

//   const handleTelegramShare = () => {
//     const telegramUrl = `https://t.me/share/url?url=${encodeURIComponent(giveawayUrl)}&text=${encodeURIComponent(shareText)}`;
//     window.open(telegramUrl, "_blank");
//   };

//   const handleEmailShare = () => {
//     const subject = encodeURIComponent("Check out this giveaway on Kuku!");
...
//           <h2 className="text-xl font-bold text-black">Share Giveaway</h2>
//           <button
//             onClick={onClose}
...
//                 {giveawayDetails?.items?.join(", ") || "Various items available"}
//               </p>
//               <p className="text-xs text-gray-400">
//                 {giveawayDetails?.numberOfItems || "Multiple"} items • {giveawayDetails?.city || "Location"}
//               </p>
//             </div>
//           </div>
...
//             <label className="text-sm font-medium text-gray-700">Giveaway Link</label>
//             <div className="flex items-center space-x-2">
//               <input
//                 type="text"
//                 value={giveawayUrl}
//                 readOnly
//                 className="flex-1 px-3 py-2 text-sm bg-gray-50 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
//               />
//               <button
//                 onClick={handleCopyLink}
//                 disabled={!giveawayUrl}
...
//                 disabled={!giveawayUrl}
//                 className="flex items-center space-x-3 p-3 bg-green-50 hover:bg-green-100 rounded-lg transition-colors group disabled:opacity-50 disabled:cursor-not-allowed"
//               >
...
//                 disabled={!giveawayUrl}
//                 className="flex items-center space-x-3 p-3 bg-pink-50 hover:bg-pink-100 rounded-lg transition-colors group disabled:opacity-50 disabled:cursor-not-allowed"
//               >
...
//                 disabled={!giveawayUrl}
//                 className="flex items-center space-x-3 p-3 bg-purple-50 hover:bg-purple-100 rounded-lg transition-colors group disabled:opacity-50 disabled:cursor-not-allowed"
//               >
...
//                 disabled={!giveawayUrl}
//                 className="flex items-center space-x-3 p-3 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors group disabled:opacity-50 disabled:cursor-not-allowed"
//               >
...
//                 disabled={!giveawayUrl}
//                 className="flex items-center space-x-3 p-3 bg-gray-50 hover:bg-gray-100 rounded-lg transition-colors group disabled:opacity-50 disabled:cursor-not-allowed"
//               >
...
const GiveAwayShareModal = ({ isOpen, onClose, giveawayDetails }) => {
  ...
  // Set giveaway share URL when component mounts on client
  useEffect(() => {
    if (typeof window !== "undefined") {
      const url = "https://kuku.moshimoshi.cloud";
      setGiveawayUrl(url);
    }
  }, []);

  const shareText = "Just gave my pre-loved items a second life with KuKu ♻️💛 Donate, repurpose, recycle — join me!";

  const handleCopyLink = async () => {
    // Method 1: Try modern clipboard API if document is focused
    if (document.hasFocus() && navigator.clipboard) {
      try {
        await navigator.clipboard.writeText(`${shareText} ${giveawayUrl}`);
        setCopySuccess(true);
        setTimeout(() => setCopySuccess(false), 2000);
        return;
      } catch (err) {
    ...
    try {
      const textArea = document.createElement("textarea");
      textArea.value = `${shareText} ${giveawayUrl}`;
  ...
  const handleEmailShare = () => {
    const subject = encodeURIComponent("Join me in giving items a second life with KuKu!");
    const body = encodeURIComponent(`${shareText}\n\n${giveawayUrl}`);
