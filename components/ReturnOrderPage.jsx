...
//     // Check if URL has /track-return-status path
...
//         const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/order/OrderDetails/${cleanOrderId}`);
...
//   if (error) return <div className="text-center p-4 md:p-8 text-red-500">Error: {error}</div>;
//   if (!order || !order.products || order.products.length === 0) {
//     return <div className="text-center p-4 md:p-8">No order or products found</div>;
//   }

//   const steps = [
//     { icon: "/bagadd.png", text: "Your Details", active: currentStep === 1 },
//     { icon: "/returnr.png", text: "Reason for Return", active: currentStep === 2 },
//     { icon: "/paymenthand.png", text: "Payment Reimbursement", active: currentStep === 3 },
//     { icon: "/review.png", text: "Review & Submit", active: currentStep === 4 },
//   ];

//   const handleNext = () => {
//     if (currentStep === 2 && (!selectedCondition || selectedReasons.length === 0)) {
...
//     const orderId = searchParams.get("orderId");
//     const returnData = {
//       orderStatus: "Buyer Wanted Return Item",
//       processingDetails: {
//         buyerReturnDetail: {
...
//     try {
//       const orderResponse = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/order/updateOrder/${orderId}`, {
//         method: "PATCH",
...
//     // Navigate to track-return-status route with orderId
...
//         <h3 className="font-medium text-base sm:text-lg">{product.productName || "Unknown Product"}</h3>
//         <p className="text-xs sm:text-sm text-gray-500 mt-1">{product.product.description || "No description"}</p>
//         <p className="text-xs sm:text-sm text-gray-600 mt-1">Brand: {product.product.brand?.brandName || "Unknown Brand"}</p>
//         <div className="flex items-center gap-2 mt-2">
//           <span className="font-medium text-base sm:text-lg">AED{order.finalAmount.toFixed(2)}</span>
//           {order.discount > 0 && (
//             <>
//               <span className="text-gray-500 text-xs sm:text-sm line-through">AED{order.totalAmount.toFixed(2)}</span>
...
//             <span className="text-xs sm:text-sm text-gray-500">{product.product.condition?.conditionName || "Unknown Condition"}</span>
//           </div>
//         </div>
//       </div>
//     </div>
...
//         <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-4 sm:mb-6 md:mb-8">Track Return Status</h1>
...
//                   </div>
//                   <div>
//                     <p className="font-medium text-sm sm:text-base">Return started</p>
//                     <p className="text-[#E4086F] text-xs sm:text-sm">Awaiting admin processing</p>
//                   </div>
//                 </div>
//                 <div className="flex items-start gap-3 mb-6 sm:mb-8">
...
//                     Drop off the item by {new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toLocaleDateString()}
//                   </p>
//                 </div>
...
//                 <Image src="/delivery_icon.png" alt="Delivery" width={28} height={28} className="flex-shrink-0" />
//                 <div>
//                   <span className="text-[#E4086F] font-medium text-sm sm:text-base">{order.orderStatus || "Buyer Wanted Return Item"}</span>
//                   <span className="text-gray-400 text-xs sm:text-sm block">
//                     On {new Date(order.processingDetails?.buyerReturnDetail?.returnInitiatedAt || order.paidAt).toLocaleDateString()}
//                   </span>
//                 </div>
//               </div>
...
//   return (
//     <div className="max-w-6xl mx-auto p-4 sm:p-6 md:p-8">
//       <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-4 sm:mb-6 md:mb-8">Return My Order(s)</h1>
//       <div className="flex flex-col lg:flex-row gap-6 lg:gap-12">
//         {/* Steps Sidebar - Hidden on mobile, shown as horizontal on tablet, vertical on desktop */}
...
//                 <div key={index} className="flex items-center gap-3 relative flex-shrink-0 lg:flex-shrink lg:mb-0">
//                   <div
//                     className={`w-10 h-10 ${step.active || index + 1 < currentStep ? "bg-[#B25CF3]" : "border-2 border-gray-200"} flex items-center justify-center flex-shrink-0`}
//                   >
//                     <Image
//                       src={step.icon}
//                       alt={step.text}
...
//                     />
//                   </div>
//                   <span className={`text-xs sm:text-sm whitespace-nowrap lg:whitespace-normal ${step.active ? "text-[#E4086F] font-medium" : "text-gray-500"}`}>
//                     {step.text}
//                   </span>
//                   {index < steps.length - 1 && (
//                     <div
//                       className={`hidden lg:block absolute left-5 top-10 w-0.5 h-14 ${index + 1 < currentStep ? "bg-[#B25CF3]" : "bg-gray-200"}`}
//                     />
//                   )}
//                 </div>
//               ))}
...
//             </span>
//             <span className="text-xs text-gray-500">{steps[currentStep - 1].text}</span>
//           </div>
//           <div className="mt-2 flex gap-1">
//             {steps.map((_, index) => (
//               <div
//                 key={index}
//                 className={`h-1 flex-1 rounded-full ${index + 1 <= currentStep ? "bg-[#E4086F]" : "bg-gray-200"}`}
//               />
//             ))}
//           </div>
//         </div>
...
//               <h2 className="text-lg sm:text-xl font-medium mb-4 sm:mb-6">Order Details</h2>
//               <div className="flex flex-col gap-4 sm:gap-8">
//                 <div className="flex-shrink-0">
//                   <div className="flex items-start gap-3 sm:gap-4 mb-4">
//                     <Image src="/delivery_icon.png" alt="Delivery" width={28} height={28} className="flex-shrink-0" />
//                     <div>
//                       <span className="text-[#E4086F] font-medium text-sm sm:text-base">{order.orderStatus}</span>
//                       <span className="text-gray-400 text-xs sm:text-sm block">
...
//               <h3 className="text-base sm:text-lg font-medium mb-2">Select the reason for your return</h3>
//               <p className="text-xs sm:text-sm text-gray-500 mb-4 sm:mb-6">
//                 To help us process your request quickly, please answer the following questions.
//               </p>
//               <div className="space-y-6 sm:space-y-8">
//                 <div>
//                   <h4 className="text-xs sm:text-sm font-medium mb-3 sm:mb-4">What is the product&apos;s current condition?</h4>
...
//                 </div>
//                 <div>
//                   <h4 className="text-xs sm:text-sm font-medium mb-3 sm:mb-4">What is the primary reason for returning the product?</h4>
//                   <div className="space-y-2 sm:space-y-3">
//                     {[
...
//               <h2 className="text-lg sm:text-xl font-medium mb-4 sm:mb-6">Choose the method for receiving payment</h2>
...
//                         <span className="font-medium text-sm sm:text-base">Refund</span>
//                       </div>
//                       <p className="text-xs sm:text-sm text-gray-500 mt-1">Refund processed within 7 business days.</p>
//                     </div>
//                   </div>
//                 </label>
//               </div>
...
//           {currentStep === 4 && (
//             <div className="bg-white rounded-lg p-4 sm:p-6 md:p-8 shadow-md">
//               <h2 className="text-lg sm:text-xl font-medium mb-4 sm:mb-6">Review your return request</h2>
//               <div className="space-y-4 sm:space-y-6">
//                 {renderProductDetails(order.products[0])}
//                 <div className="space-y-3 sm:space-y-4 mt-6 sm:mt-8">
//                   <div>
//                     <h4 className="text-xs sm:text-sm font-medium text-gray-500">Current state of the product:</h4>
//                     <p className="mt-1 text-sm sm:text-base">{selectedCondition || "Not selected"}</p>
//                   </div>
//                   <div>
//                     <h4 className="text-xs sm:text-sm font-medium text-gray-500">Main reason for returning the product:</h4>
//                     <p className="mt-1 text-sm sm:text-base">{selectedReasons.join(", ") || "Not selected"}</p>
//                   </div>
//                   <div>
//                     <h4 className="text-xs sm:text-sm font-medium text-gray-500">Method for receiving payment:</h4>
//                     <p className="mt-1 text-sm sm:text-base">Refund to original payment method</p>
//                   </div>
//                 </div>
//                 <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 sm:gap-4 mt-6 sm:mt-8">
//                   <button
...
//                 <svg className="w-7 sm:w-8 h-7 sm:h-8 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
//                   <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
//                 </svg>
//               </div>
//               <h3 className="text-lg sm:text-xl font-medium mb-2">Your return request has been successfully received.</h3>
