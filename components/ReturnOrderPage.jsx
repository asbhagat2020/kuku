...
const ReturnOrderPage = () => {
  ...
  const handleSubmit = async () => {
    const orderId = searchParams.get("orderId");
    const returnData = {
      orderStatus: "Out for reverse delivery",
      returnDetails: {
        condition: selectedCondition,
        reasons: selectedReasons,
        pickupMethod: selectedPickupMethod,
    ...
    // Prepare reverse shipment payload
    const reversePayload = {
      delivery_type: "Next Day",
      load_type: "Non-document",
      consignment_type: "REVERSE",
      description: `Return for Order #${orderId} due to ${selectedReasons.join(", ")}`,
      weight: order.products.reduce((sum, p) => sum + (p.weight || 1), 0) || 2,
      payment_type: "PREPAID",
      cod_amount: 0,
      num_pieces: order.products.length || 1,
      customer_reference_number: orderId,
      origin_address_name: order.shippingAddress.addressName,
      origin_address_mob_no_country_code: "971",
      origin_address_mobile_number: order.shippingAddress.phoneNumber,
      origin_address_house_no: order.shippingAddress.street,
      origin_address_building_name: order.shippingAddress.apartmentName,
      origin_address_area: order.shippingAddress.city,
      origin_address_landmark: order.shippingAddress.apartmentDetails,
      origin_address_city: order.shippingAddress.city,
      origin_address_type: "Normal",
      destination_address_name: order.products[0]?.product?.seller?.name || "Kuku Warehouse",
      destination_address_mob_no_country_code: "971",
      destination_address_mobile_number: order.products[0]?.product?.seller?.phone || "505186403",
      destination_address_house_no: order.shippingAddress.street,
      destination_address_building_name: "Kuku Warehouse",
      destination_address_area: "Warehouse Area",
      destination_address_landmark: "Warehouse Landmark",
      destination_address_city: "Dubai",
      destination_address_type: "Normal",
      pickup_date: new Date().toISOString().split("T")[0],
      rejectionReason: selectedReasons.join(", "),
      uploadphotos: order.qualityCheckProof?.[0] || "",
    };
    ...
    try {
      ...
      // Step 2: Create reverse shipment
      const shipmentResponse = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/shipment/createReverseShipment`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(reversePayload),
      });
      if (!shipmentResponse.ok) throw new Error("Failed to create reverse shipment");
