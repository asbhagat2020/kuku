...
export const FilterProvider = ({ children }) => {
  ...
  const fetchProductsWithFilters = useCallback(async (parentCategory, categoryName, subCategoryName) => {
    ...
    try {
      ...
      if (parentCategory) params.push(`parentCategory=${encodeURIComponent(parentCategory)}`);
      if (categoryName) params.push(`categoryName=${encodeURIComponent(categoryName)}`);
      if (subCategoryName) params.push(`subCategoryName=${encodeURIComponent(subCategoryName)}`);
  ...
  useEffect(() => {
    ...
    const hasActiveFilters = filters.price.length > 0 || filters.size.length > 0 || filters.condition.length > 0 ||
      (filters.category.length > 0 && !filters.category.includes(categoryInfo.parentCategory));

    if (!hasActiveFilters && categoryInfo.parentCategory && filters.category.includes(categoryInfo.parentCategory)) {
    ...
    const filtered = baseApiProducts.filter(product => {
      ...
      if (filters.category.length > 0) {
        const nonParentCategoryFilters = filters.category.filter(cat => cat !== categoryInfo.parentCategory);
        if (nonParentCategoryFilters.length > 0 && 
  ...
  }, [filters, baseApiProducts, categoryInfo.parentCategory]);
  ...
  const currentProducts = baseApiProducts.length > 0 ? apiFilteredProducts : filteredProducts;
